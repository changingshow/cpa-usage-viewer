...
describe('UsagePage active tab auto-refresh guard', () => {
  ...
  it('keeps Overview auto-refresh enabled and does not auto-refresh other tabs', () => {
    ...
    expect(shouldAutoRefreshUsageTab({ activeTab: 'pricing', eventsPage: 1 })).toBe(false);
  });
});
...
describe('UsagePage time range options', () => {
  it('includes all data, rolling 24h, local Today, and 30d ranges', () => {
    const options = getTimeRangeOptions((key) => `translated:${key}`);

    expect(options.map((option) => option.value)).toEqual(['all', '4h', '8h', '12h', '24h', 'today', '7d', '30d', 'custom']);

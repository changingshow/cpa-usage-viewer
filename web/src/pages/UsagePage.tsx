...
const USAGE_TAB_OPTIONS = ['overview', 'events', 'analysis', 'pricing'] as const;
...
const parseStatusDate = (value?: string): Date | null => {
  if (!value) return null;
  const parsed = new Date(value);
  return Number.isNaN(parsed.getTime()) ? null : parsed;
};

const formatMetaDateTime = (date: Date): string => {
  const pad = (value: number) => String(value).padStart(2, '0');
...
export function UsagePage({ onAuthRequired }: { onAuthRequired?: () => void }) {
  ...
  const refreshActiveTab = useCallback(async () => {
    ...
    await loadUsage();
  }, [activeTab, loadAnalysis, loadEventFilterOptions, loadEvents, loadPricing, loadUsage]);
  ...
  const lastSyncAt = useMemo(() => {
    return parseStatusDate(status?.last_run_at);
  }, [status?.last_run_at]);
  const fileDataRangeLabel = useMemo(() => {
    const start = parseStatusDate(status?.data_range_start);
    const end = parseStatusDate(status?.data_range_end);
    if (!start || !end) return '';
      ...
      <div className={styles.pageFrame}>
        <header className={styles.topBar}>
          ...
          <span className={styles.eyebrow}>
            CPA Usage<span className={styles.eyebrowAccent}>Viewer</span>
          </span>
          </div>
